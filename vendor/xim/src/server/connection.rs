...
use xim_parser::{
    ...
    Point, Rectangle, Request, XimWrite,
};
...
pub struct InputContext {
    ...
    preedit_area: Option<Rectangle>,
    preedit_area_needed: Option<Rectangle>,
    preedit_spot: Point,
    preedit_line_space: Option<u32>,
...
impl InputContext {
    pub fn new(
        ...
        Self {
            ...
            preedit_area: None,
            preedit_area_needed: None,
            preedit_spot: Point { x: 0, y: 0 },
            preedit_line_space: None,
    ...
    pub fn preedit_area(&self) -> Option<Rectangle> {
        self.preedit_area.clone()
    }

    pub fn preedit_area_needed(&self) -> Option<Rectangle> {
        self.preedit_area_needed.clone()
    }

    pub fn preedit_line_space(&self) -> Option<u32> {
        self.preedit_line_space
    }
                            ...
                            match attrs::get_name(attr.id) {
                                Some(AttributeName::Area) => {
                                    if let Ok(area) = xim_parser::read(&attr.value) {
                                        log::debug!("Area: {:?}", area);
                                        ic.preedit_area = Some(area);
                                    }
                                }
                                Some(AttributeName::AreaNeeded) => {
                                    if let Ok(area) = xim_parser::read(&attr.value) {
                                        log::debug!("Area needed: {:?}", area);
                                        ic.preedit_area_needed = Some(area);
                                    }
                                }
                                ...
                                Some(AttributeName::LineSpace) => {
                                    if let Ok(line_space) = xim_parser::read(&attr.value) {
                                        log::debug!("Line space: {:?}", line_space);
                                        ic.preedit_line_space = Some(line_space);
                                    }
                                }
                                name => {
